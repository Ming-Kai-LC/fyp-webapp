...
class PatientAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Medical Information", {"fields": ("medical_history", "current_medications")}),
        ("Contact Information", {"fields": ("emergency_contact", "address")}),
        (
            "Statistics",
            {"fields": ("total_xrays", "covid_count"), "classes": ("collapse",)},
        ),
        (
            "Timestamps",
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
    ...
    def total_xrays(self, obj):
        ...
        url = (
            reverse("admin:detection_xrayimage_changelist")
    ...
    def covid_count(self, obj):
        ...
        if count > 0:
            return format_html(
                '<span style="color: red; font-weight: bold;">{} COVID+ cases</span>',
                count,
            )
...
class PredictionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("X-Ray & Patient Information", {"fields": ("xray", "xray_preview")}),
        (
    ...
    def agreement_display(self, obj):
        ...
        return format_html(
            '<span style="color: {};">{} {}/6 models</span>', color, icon, agreement
        )
    ...
    def mark_as_validated(self, request, queryset):
        ...
        updated = queryset.update(
            is_validated=True, reviewed_by=request.user, validated_at=timezone.now()
        )
