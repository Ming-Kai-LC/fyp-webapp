...
class ModelEnsemble:
    ...
    def _get_weights_path(self, weights_filename):
        ...
        possible_paths = [
            ...
            (
                os.path.join(settings.STATIC_ROOT, "ml_models", weights_filename)
        ...
        try:
            ...
            if weights_path:
                try:
                    ...
                    logger.warning(
                        f"   Using pretrained ImageNet initialization instead"
                    )
                    # Fallback: use ImageNet pretrained
                    model = timm.create_model(
                        config["name"], pretrained=True, num_classes=4
                    )
            else:
    ...
    def predict_single_model(self, model_name, image_path):
        ...
        logger.info(
            f"   ✅ {model_name}: {result['prediction']} ({result['confidence']:.2f}%) in {inference_time:.3f}s"
        )
    ...
    def predict_all_models(self, image_path):
        ...
        logger.info(
            f"   Best Model: {best_model[0]} ({best_model[1]['confidence']:.2f}%)"
        )
