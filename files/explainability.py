...
try:
    ...
    logger.warning("⚠️ pytorch-grad-cam not installed. Run: pip install pytorch-grad-cam")
...
class ExplainabilityEngine:
    ...
    def generate_gradcam(self, image_path, target_class_idx, model_name='crossvit', save_path=None):
        """
        ...
        try:
            ...
            visualization = show_cam_on_image(image_np_resized, grayscale_cam, use_rgb=True)
    ...
    def generate_dual_branch_visualization(self, image_path, save_dir=None):
        ...
        try:
            ...
            if save_dir:
                ...
                cv2.imwrite(large_path, cv2.cvtColor(large_with_grid, cv2.COLOR_RGB2BGR))
                cv2.imwrite(small_path, cv2.cvtColor(small_with_grid, cv2.COLOR_RGB2BGR))
    ...
    def generate_comparison_visualization(self, image_path, predictions, save_path=None):
        """
        ...
        try:
            ...
            # Add text to image
            cv2.putText(original, prediction_text, (10, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
            cv2.putText(original, confidence_text, (10, 60),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
...
def generate_explainability_report(model, device, image_path, prediction_result, model_name='crossvit'):
    """
    ...
    try:
        ...
        class_idx = class_names.index(predicted_class) if predicted_class in class_names else 2
        
        # Generate save directory
        save_dir = os.path.join(settings.MEDIA_ROOT, 'explainability', 
        ...
        if 'crossvit' in model_name.lower():
            dual_branch = engine.generate_dual_branch_visualization(image_path, save_dir)
