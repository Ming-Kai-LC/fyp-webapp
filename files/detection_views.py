...
from .forms import XRayUploadForm, UserRegistrationForm, PatientProfileForm, DoctorNotesForm
...
def home(request):
    """Landing page"""
    context = {
        'total_predictions': Prediction.objects.count() if request.user.is_authenticated else 0,
        'total_patients': Patient.objects.count() if request.user.is_authenticated else 0,
    }
            ...
            try:
                ...
                if patient_id:
                    ...
                    if hasattr(request.user, 'patient_info'):
                        ...
                        messages.error(request, 'Please select a patient or create patient profile.')
                ...
                if model_ensemble is None:
                    messages.error(request, 'ML Engine not initialized. Please contact admin.')
                    return redirect('detection:doctor_dashboard')
                ...
                prediction = Prediction.objects.create(
                    ...
                    # ResNet-50
                    resnet50_prediction=results['individual_results']['resnet50']['prediction'],
                    resnet50_confidence=results['individual_results']['resnet50']['confidence'],
                    # DenseNet-121
                    densenet121_prediction=results['individual_results']['densenet121']['prediction'],
                    densenet121_confidence=results['individual_results']['densenet121']['confidence'],
                    # EfficientNet-B0
                    efficientnet_prediction=results['individual_results']['efficientnet']['prediction'],
                    efficientnet_confidence=results['individual_results']['efficientnet']['confidence'],
                    # ViT-Base
...
def explain_prediction(request, prediction_id):
    ...
    if not prediction.gradcam_heatmap:
        try:
            logger.info(f"🔍 Generating explainability for prediction {prediction_id}...")
...
def prediction_history(request):
    ...
    if request.user.profile.is_doctor():
        ...
        predictions = Prediction.objects.filter(
            xray__patient__user=request.user
...
def patient_dashboard(request):
    ...
    my_predictions = Prediction.objects.filter(
        xray__patient=patient
    ).select_related('xray', 'reviewed_by').order_by('-created_at')[:10]
